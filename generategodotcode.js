class Annotation {
    ...
    constructor(x, y, title, description) {
      this.x = x;
      this.y = y;
      this.title = title;
...
class Picture {
    ...
    constructor(filename, title, description, method, author, year, period, origin, annotations, source) {
...
io += `
func ${picture.title.replaceAll(",", "").replaceAll(".", "").replaceAll("(", "").replaceAll(")", "").replaceAll(":", "").replaceAll("- ", "").toLowerCase().replaceAll("ž", "z").replaceAll("č", "c").replaceAll("š", "s").split(" ").join("_")}(body: Node):
    self.bbcode_text = """
    [b]${picture.title}[/b]
    ${picture.author.join(" ")}
    [i]${picture.period}[/i]
